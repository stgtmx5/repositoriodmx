...
function CrearPanel(){
	...
	Application.UIShortCuts.CreateAction("P_Ventas_A99","Explosión de insumos",0,"","","","UIVentas.ExplosionInsumos",0,"","","",0);
	...
	Application.UIShortCuts.CreateAction("_P_Ventas_N01","Totales CFDI",0,"","","","UIVentas.VistaPreviaCFDI",0,"","","",0);
...
if (Brw==null)
	{
		...
		// Brw.CmdLeftEdit="UIInventario.EdLinea";
		// Brw.CmdLeftDelete = "UIInventario.ElLinea";
		//Brw.CmdLeftDblClick=Brw.CmdEdit;
		...
		Brw.AddButton("Explosión de insumos","P_Ventas_A99");
		...
		//Brw.SetCaptionByFieldName("Const","Documento");
...
function GuardarCerrar (objAction){
	...
	try{
		...
		//if (
		//eBasic.eMsgbox("¿Desea crear un nuevo documento?", 4)==7){
...
var ThisObj;
	...
	if (Venta.ShowDlgFormaPago(ThisObj.ThisObj,0,ThisObj.OmitirExistencia,ThisObj.ParentVenta,true,ThisObj.ColDocVentas,ThisObj.ColConsignaciones)){        		   
		...
		if(parseInt(ThisObj.GetIstimbrado("deminus"))==0){				
			return -1;
		}
...
if (Brw==null) return 0;
	...
	if (PKVenta==null){
		eBasic.eMsgbox("Debe seleccionar un documento de venta");
		return 0;
	}
	ThisObj = Application.NewObject("EDOFx.Venta");
	
	if (ThisObj.LoadFromADOConnection (PKVenta,"", Application.adoCnn,2))
	{
		PKA=ThisObj.ICConsumo.FK_IAlmacen;
		...
		obj.DlgExplosion(Application.adoCnn,PKVenta,PKA);
	}
}
