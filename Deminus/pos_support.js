//funciones de soporte utilizadas en el punto de venta.
...
function ReciboCliente(PKDCxC)
{
	...
	sql="SELECT DCxC.Sys_DTCreated,DCxC.Fecha, DCxC.Haber, DCxC.Notas, DCxC.Referencia, Cliente.Nombre FROM Cliente INNER JOIN DCxC ON Cliente.Sys_PK = DCxC.ICliente WHERE DCxC.Sys_PK="+PKDCxC;
	...
	if(R.EOF && R.BOF){
		eBasic.eMsgbox(ErrDesc + "(No se encontró el elemento con clave primaria= "+PKDCxC+")");
		return 0;
	}
...
function ReciboProveedor(PKDCxP)
{
	...
	if(R.EOF && R.BOF){
		eBasic.eMsgbox(ErrDesc + "(No se encontró el elemento con clave primaria= "+PKDCxP+")");
		return 0;
	}
	
	//return ImprimirVale(R("IMovCaja").Value,"PAGO A PROVEEDOR",ErrDesc);	
